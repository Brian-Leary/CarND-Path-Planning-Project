...
//int NextWaypoint(double x, double y, double theta, const vector<double> &maps_x, const vector<double> &maps_y)
{
    ...
    */
	...
	//If the inner product of v and h is positive then we are behind the waypoint so we do not need to increment
	//closestWaypoint, otherwise we are beyond the waypoint and we need to increment closestWaypoint.
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			for (int i = 0; i < sensor_fusion.size(); i++) {
				...
				if (d < (2 + 4*lane + 2) && d > (2 + 4*lane -2) ) {
					...
					if ((check_car_s > car_s) && ((check_car_s - car_s) < 30) ) {
						// Work out the logic here; lower the reference velocity here so we don't crash
			...
			// Start with all the previous points from last time
			...
			for (int i = 1; i <= 50-previous_path_x.size(); i++) {
				// Convert to meters per second (1mph = 0.447m/s * 5 = 2.24)
			    double N = (target_dist / (.02*ref_vel/2.24));
			    double x_point = x_add_on + (target_x)/N;
			    // Get spline reference
			    ...
			    // Rotate back to global coordinates after rotating to local coordinates earlier
			...
			double dist_inc = 0.4;
			for(int i = 0; i < 50; i++) {
				// Use i+1 instead of i so our first point isn't where the car is currently positioned
